...
function siteUrl() {
  return (process.env.SITE_URL || "https://clq-lille.netlify.app").replace(/\/+$/, "");
}
...
export async function mailStaffPoiProposal({
  ...
  const html = `
    <h2>Nouvelle proposition de POI — CLQ Lille</h2>
  ...
  await tx.sendMail({
    ...
    to: staffEmail(),
    subject: `[CLQ Lille] POI a valider : ${name}`,
    html,
    attachments,
  });
  return true;
...
export async function mailSubmitterResult({ to, name, approved }) {
  ...
  const subject = approved
    ? `[CLQ Lille] Votre point d'interet a ete publie`
    : `[CLQ Lille] Proposition de POI non retenue`;
  const body = approved
    ? `Bonjour,\n\nVotre proposition « ${name} » a ete validee par notre equipe et est maintenant visible dans l'application CLQ Lille.\n\nMerci pour votre contribution !`
