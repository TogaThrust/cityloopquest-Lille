...
function getAudioHeardIndices() {
    try {
        const saved = localStorage.getItem('lille_audio_heard_indices');
        if (saved) {
            const parsed = JSON.parse(saved);
            if (Array.isArray(parsed)) return parsed.map(Number).filter(n => !Number.isNaN(n));
        }
    } catch (_) { /* ignore */ }
    const legacy = localStorage.getItem('lille_audio_clicked_index');
...
function getAudioCompletedIndices() {
    try {
        const saved = localStorage.getItem('lille_audio_completed_indices');
        if (saved) {
            const parsed = JSON.parse(saved);
...
function markAudioHeardForIndex(idx) {
    ...
    const arr = [...set];
    try {
        localStorage.setItem('lille_audio_heard_indices', JSON.stringify(arr));
        localStorage.setItem('lille_audio_clicked_index', String(idx));
...
function unlockPointNavigationAfterAudioStart(idx) {
    ...
    if (isTourFreeRoam() && atLastPoint) return;
    if (!isPointUnlockedForNext(i)) return;
    nextButton.disabled = false;
    nextButton.removeAttribute('disabled');
    nextButton.setAttribute('data-audio-unlocked', 'true');
...
function syncNextButtonState() {
    try {
        ...
        let shouldDisable = isTourFreeRoam()
            ? atLastPoint
        ...
        if (!shouldDisable) {
            ...
            nextButton.removeAttribute('data-audio-unlocked');
        }
    } catch (err) {
...
let completedQuizQuestions = (() => {
    const saved = localStorage.getItem("lille_completedQuizQuestions");
...
function getRouteDestination() {
    if (localStorage.getItem('museumMode') === 'true') {
        try {
            ...
            if (museum && Number.isFinite(lat) && Number.isFinite(lng)) {
                return { ...museum, lat, lng };
            }
        } catch (e) {
            /* ignore */
        }
...
function resetTourProgressForNewCircuit() {
  currentIndex = 0;
  localStorage.setItem('lille_currentIndex', '0');
  localStorage.removeItem('tourCompleted');
  localStorage.removeItem('lille_audio_heard_indices');
  localStorage.removeItem('lille_audio_clicked_index');
  localStorage.removeItem('lille_audio_completed_indices');
}
...
function encodeAssetPath(assetPath) {
    if (/^(https?:|data:|blob:)/i.test(String(assetPath || ""))) {
...
function getPointImagePathCandidates(location) {
    ...
    function addLocal(path) {
        if (!path || seen.has(path) || /^https?:\/\//i.test(String(path))) return;
        seen.add(path);
    ...
    function addRemote(path) {
        if (!path || seen.has(path)) return;
        seen.add(path);
        remotePaths.push(path);
    }

    if (location.image && !/^https?:\/\//i.test(String(location.image))) {
        addLocal(location.image);
    } else if (location.image) {
        addRemote(location.image);
    }
    if (location.fallbackImage) {
        addLocal(location.fallbackImage);
    }

    const rawName = (location.name || "").trim();

    if (location.audio) {
        const audioBase = location.audio
            .replace(/^audio\//i, "")
            .replace(/\.mp3$/i, "");
        addLocal(`images/${audioBase}.jpg`);
        addLocal(`images/points interets/${audioBase}.jpg`);
        addLocal(`images/${audioBase}.png`);
        addLocal(`images/points interets/${audioBase}.png`);
        addLocal(`images/${audioBase}.jpeg`);
        addLocal(`images/points interets/${audioBase}.jpeg`);
    }
    ...
    if (rawName) {
        ...
        const slugBase = String(rawName)
            .normalize("NFD")
            .replace(/[\u0300-\u036f]/g, "")
            .toLowerCase()
            .replace(/[^a-z0-9]+/g, "-")
            .replace(/(^-|-$)/g, "");
        if (slugBase) bases.push(slugBase);
    }

    for (const base of bases) {
        for (const folder of ["images/points interets", "images", "images/POI explorer"]) {
            for (const ext of POI_IMAGE_EXTENSIONS) {
                addLocal(`${folder}/${base}${ext}`);
            }
        }
    }

    if (location.remoteImage) {
        addRemote(location.remoteImage);
    }

    return [...localPaths, ...remotePaths, ...svgLocals];
}
...
function normalizeDescriptionLang(lang) {
    let code = String(lang || 'fr').toLowerCase();
    if (code === 'jp') code = 'ja';
    if (code === 'zh') code = 'cn';
    return code;
}

function stripLegacyPoiTextHeader(text) {
    return String(text || '')
        .replace(/^Texte pour .+?\.\s*\n+/i, '')
        .trim();
}
...
function getDescription(locationName, language = null) {
    ...
    const lang = normalizeDescriptionLang(
        language || (window.translationManager ? window.translationManager.getCurrentLanguage() : 'fr'),
    );
    const description = descriptionsData[lang]?.[locationName]
        || (lang === 'ja' ? descriptionsData.jp?.[locationName] : null)
        || (lang === 'cn' ? descriptionsData.zh?.[locationName] : null);
...
function getAudioPath(baseAudioPath, language = null) {
    let lang = language || (window.translationManager ? window.translationManager.getCurrentLanguage() : 'fr');
...
function playExclusiveAudio(src, textFile = null, imageElement = null, originalImageSrc = null) {
    ...
    if (textFile && imageElement && textContainer) {
        ...
        fetch(textFile)
            ...
            .then((text) => {
                if (isHtmlPayload(text)) {
                    console.warn(`Réponse HTML inattendue pour ${textFile} — texte POI ignoré`);
                    return;
                }
...
function snapshotTourStateForExternalPage() {
    localStorage.setItem('lille_currentIndex', String(currentIndex));
    localStorage.setItem('lille_score', String(score));
    try {
...
function bootstrapMainTourDisplay() {
    if (localStorage.getItem("museumMode") === "true") return;
    updatePointPhotoForCurrentIndex();
    const dest = getRouteDestination();
    const display = document.getElementById("location-name");
    if (display && dest && dest.name) {
        const nextPoint = window.translationManager && window.translationManager.isLoaded
            ? window.translationManager.translate("next_point")
            : "Prochain point :";
        display.textContent = `${nextPoint} ${dest.name}`;
    }
    if (!map) return;
    const livePos = getCurrentUserLatLng();
    const savedPos = readLastKnownUserPosition();
    const origin = livePos || savedPos;
    if (origin) {
        if (savedPos && !livePos) updateUserMarker(savedPos);
        calculateRouteFromPosition(origin, livePos ? "Votre position" : "Dernière position");
    } else if (typeof startPoint !== "undefined" && startPoint) {
        calculateRouteFromPosition(startPoint, "Point de départ");
    }
}
...
function applyMapViewForGuidance(userPos) {
    if (!map) return;
    const lat = Number(userPos && userPos.lat);
    const lng = Number(userPos && userPos.lng);
    if (!Number.isFinite(lat) || !Number.isFinite(lng)) {
        console.warn('[CLQ] Position utilisateur invalide, recentrage carte ignoré:', userPos);
        return;
    }
    const center =
        typeof google !== 'undefined' && google.maps && google.maps.LatLng
...
function advanceToNextPoint() {
    ...
    if (currentIndex === filteredLocations.length - 2) {
        ...
        const visitedPoints = JSON.parse(localStorage.getItem('lille_visitedPoints') || '[]');
        if (!visitedPoints.includes(currentIndex)) {
            visitedPoints.push(currentIndex);
            localStorage.setItem('lille_visitedPoints', JSON.stringify(visitedPoints));
        }
        ...
        steps = [];
        currentStepIndex = 0;
        localStorage.setItem("lille_currentIndex", currentIndex);
        localStorage.setItem("lille_score", score);
...
function getClqAccessKeysToPreserve() {
    return [
        'clq_short_code', 'clq_token', 'jwt', 'clq_has_access', 'user_version',
        'selectedLanguage', 'lang', 'pwa-installed',
        'clq_city', 'clq_entitlements', 'deviceId', 'api_base', 'entitlements',
        'orientationPermissionGranted', 'geoPermissionGranted',
    ];
}

function getClqSessionKeysToPreserve() {
    return ['webLanguageSelected'];
}
...
function hasStoredClqAccess() {
    try {
        if (localStorage.getItem('clq_has_access') === '1') return true;
        if (localStorage.getItem('clq_token') || localStorage.getItem('jwt')) return true;
        if (localStorage.getItem('clq_short_code')) return true;
        if (window.ClqLicenseCityGuard?.hasActiveLicenseForThisSite?.()) return true;
    } catch (_) { /* ignore */ }
    return false;
}
...
function clearTourStatePreservingAccess() {
    const saved = {};
    getClqAccessKeysToPreserve().forEach((key) => {
        const value = localStorage.getItem(key);
        if (value != null) saved[key] = value;
    });
    const savedSession = {};
    getClqSessionKeysToPreserve().forEach((key) => {
        const value = sessionStorage.getItem(key);
        if (value != null) savedSession[key] = value;
    });
    localStorage.clear();
    Object.entries(saved).forEach(([key, value]) => {
        localStorage.setItem(key, value);
    });
    Object.entries(savedSession).forEach(([key, value]) => {
        sessionStorage.setItem(key, value);
    });
}

function redirectToParcoursWithLanguage() {
...
async function handleResetWithCodeCheck() {
    ...
    clearTourStatePreservingAccess();
    if (savedOrientationPerm) {
        localStorage.setItem('orientationPermissionGranted', savedOrientationPerm);
    }
    redirectToParcoursWithLanguage();
}
...
function getLocationAtIndex(idx) {
  const i = Number(idx);
  if (!Array.isArray(filteredLocations) || !Number.isFinite(i) || i < 0 || i >= filteredLocations.length) {
    return null;
  }
  return filteredLocations[i];
}
...
function clearQuizCompletionFromIndex(fromIdx) {
  ...
  localStorage.setItem('lille_completedQuizQuestions', JSON.stringify(completedQuizQuestions));
}
...
function updateQuizFooterButton() {
  ...
  const show = localStorage.getItem('lille_quizEnabled') === 'false';
  quizResumeBtn.style.display = show ? '' : 'none';
}

function setQuizEnabledRuntime(enabled) {
  quizEnabled = enabled;
  localStorage.setItem('lille_quizEnabled', enabled ? 'true' : 'false');
  if (enabled) localStorage.removeItem('lille_quizStopped');
...
function stopQuizPermanently() {
  ...
  localStorage.setItem('lille_quizStopped', 'true');
...
function showQuizPrompt(callback) {
  if (localStorage.getItem("lille_quizEnabled") !== null) {
    callback(localStorage.getItem("lille_quizEnabled") === "true");
    return;
  }
  const overlay = document.createElement('div');
  ...
  btnYes.style.cursor = 'pointer';
  btnYes.addEventListener('click', () => {
    localStorage.setItem("lille_quizEnabled", "true");
    document.body.removeChild(overlay);
    updateQuizFooterButton();
    callback(true);
  });
  ...
  btnNo.style.cursor = 'pointer';
  btnNo.addEventListener('click', () => {
    localStorage.setItem("lille_quizEnabled", "false");
    document.body.removeChild(overlay);
    updateQuizFooterButton();
    callback(false);
  });
...
function initializeMainLogic() {
    ...
    const savedCurrentIndex = localStorage.getItem("lille_currentIndex");
    const savedScore = localStorage.getItem("lille_score");
    const savedQuizEnabled = localStorage.getItem("lille_quizEnabled");
    const savedCompletedQuizQuestions = localStorage.getItem("lille_completedQuizQuestions");
    
    if (savedCurrentIndex !== null) {
        currentIndex = parseInt(savedCurrentIndex, 10) || 0;
    }
    const freshCircuitStart = sessionStorage.getItem('clq_fresh_circuit_start') === '1';
    if (freshCircuitStart) {
        sessionStorage.removeItem('clq_fresh_circuit_start');
        resetTourProgressForNewCircuit();
    }
    if (Array.isArray(filteredLocations) && filteredLocations.length > 0) {
        currentIndex = Math.max(0, Math.min(currentIndex, filteredLocations.length - 1));
    }
    ...
    if (forceTarget) {
        ...
        const visitedPoints = safeJsonParse(localStorage.getItem('lille_visitedPoints'), []);
    ...
    setupGoogleMapsNavButton();
        ...
        console.error("Erreur initializeMainLogic:", error);
        bootstrapMainTourDisplay();
    }
}
...
function showStyledPopup(title, message, onClose, closeTextOverride) {
    ...
    const box = document.createElement('div');
    box.style.background = 'url("images/images%20permanentes/parchemin.jpg") center/cover';
    box.style.border = '4px solid #b8860b';
    box.style.borderRadius = '18px';
    box.style.boxShadow = '0 0 24px #0008';
...
function navigateToSelfieCelebration() {
    if (window.stream) {
        try {
...
function closeVictoryOverlayAndRevealSelfieButton(overlay) {
...
function showEndOfTourPopup() {
    ...
    const msg = document.createElement('div');
    const victoryMessage = window.translationManager ? window.translationManager.translate('victory_message') : "C'est la dernière étape de votre parcours !<br><br>Bravo, vous avez terminé le parcours !<br><br>À l'arrivée, si vous le souhaitez, vous pourrez prendre un selfie !";
    msg.innerHTML = victoryMessage;
    msg.style.marginBottom = '16px';
    msg.style.color = '#222';
    box.appendChild(msg);

    const selfieInvite = document.createElement('div');
    const selfieInviteText = window.translationManager
        ? window.translationManager.translate('victory_selfie_invite')
        : '📸 Célébrez votre victoire : prenez un selfie souvenir pour garder une trace de ce parcours !';
    selfieInvite.textContent = selfieInviteText;
    selfieInvite.style.marginBottom = '24px';
    selfieInvite.style.color = '#4b2e05';
    selfieInvite.style.fontWeight = 'bold';
    selfieInvite.style.lineHeight = '1.45';
    box.appendChild(selfieInvite);
    ...
    const selfieButtonText = window.translationManager
        ? window.translationManager.translate('selfie_button')
        : 'SELFIE';
    btnSelfie.textContent = `📸 ${selfieButtonText}`;
    btnSelfie.style.background = '#1a6b3c';
    btnSelfie.style.color = '#fff';
    btnSelfie.style.fontWeight = 'bold';
    btnSelfie.style.border = 'none';
    btnSelfie.style.borderRadius = '10px';
    btnSelfie.style.padding = '12px 24px';
    btnSelfie.style.fontSize = '1.1em';
    btnSelfie.style.cursor = 'pointer';
    btnSelfie.style.fontFamily = 'MedievalSharp, Arial, serif';
    btnSelfie.addEventListener('click', () => {
        navigateToSelfieCelebration();
    });
    ...
    btnCompris.addEventListener('click', () => {
        closeVictoryOverlayAndRevealSelfieButton(overlay);
    });

    btnRow.appendChild(btnSelfie);
    btnRow.appendChild(btnCompris);
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    nextButton.addEventListener('click', () => {
        ...
        if (localStorage.getItem("lille_quizEnabled") === null) {
            ...
            quizEnabled = localStorage.getItem("lille_quizEnabled") === "true";
            if (quizEnabled) {
                showQuizForCurrentPoint(() => advanceToNextPoint());
            } else {
                advanceToNextPoint();
